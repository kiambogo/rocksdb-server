...
static const error ERR_READ_ONLY =
...
static bool iscmd(client *c, const char *cmd) { return islstr(c, 0, cmd); }
...
static void exec_flushdb(client *c) {
  if (c->args_len != 1) {
    return client_write_error(
        c, "wrong number of arguments for 'flushdb' command");
  }
  if (readonly) {
    return client_write_error(c, "You can't write against a read only replica");
  }
...
static void exec_scan_keys(client *c, bool scan, const char *pat, int pat_len,
...
static const struct {
  ...
  command_t command;
    ...
    {"del", &exec_del},   {"flushdb", &exec_flushdb}, {"get", &exec_get},
    {"mset", &exec_mset}, {"mget", &exec_mget},       {"quit", &exec_quit},
    {"scan", &exec_scan}, {"set", &exec_set},         {"keys", &exec_keys},
...
void exec_command(client *c) {
  ...
  if (command) {
    ...
    client_write_error(
        c, client_err_unknown_command(c, c->args[0], c->args_size[0]));
    client_flush(c);
  }
}
